...
const Home = () => {
	...
	const fetchTodos = async() => {
		try{
			const response = await fetch(
				"https://playground.4geeks.com/todo/users/joangallego"
			);
	...
	const addTasks = async(label) => {
		...
		try {
			...
			const response = await fetch('https://playground.4geeks.com/todo/todos/joangallego', {
			  method: 'POST',
			  headers: {
		...
		try {
			...
			const response = await fetch(
			  `https://playground.4geeks.com/todo/todos/${id}`,
			  {
				...
				body: JSON.stringify({...taskToUpdate, is_done:
					!taskToUpdate.is_done}),
			  }
			);
